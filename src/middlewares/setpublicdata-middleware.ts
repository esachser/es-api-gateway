...
export class EsSetPublicDataMiddleware extends EsMiddleware {
    static readonly isInOut = true;
    static readonly middlewareName = 'EsSetPublicDataMiddleware';
    static readonly meta = { middleware: EsSetPublicDataMiddleware.middlewareName };
    ...
    constructor(values: any, after: boolean, api:string, nextMiddleware?: IEsMiddleware) {
        ...
        if (!_.isString(this._srcProp)){
            throw new EsMiddlewareError(EsSetPublicDataMiddleware.name, 'sourceProp MUST be string');
        }

        if (!_.isUndefined(this._ttlProp) && !_.isString(this._ttlProp)){
            throw new EsMiddlewareError(EsSetPublicDataMiddleware.name, 'ttlProp MUST be string');
        }

        if (!_.isUndefined(this._destProp) && !_.isString(this._destProp)){
            throw new EsMiddlewareError(EsSetPublicDataMiddleware.name, 'destProp MUST be string');
        }
    }
    ...
    async runInternal(context: IEsContext) {
        ...
        if (!_.isString(dest)) {
            throw new EsMiddlewareError(EsSetPublicDataMiddleware.name, `Destination (prop ${this._destProp}) MUST be string`);
        }

        const realDest = `esgateway/runtime/${configuration.env}/public/store/${dest}`;
...
export const MiddlewareCtor: IEsMiddlewareConstructor = EsSetPublicDataMiddleware;
