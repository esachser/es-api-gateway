...
export class EsGetPublicDataMiddleware extends EsMiddleware {
    static readonly isInOut = true;
    static readonly middlewareName = 'EsGetPublicDataMiddleware';
    static readonly meta = { middleware: EsGetPublicDataMiddleware.middlewareName };
    ...
    constructor(values: any, after: boolean, api:string, nextMiddleware?: IEsMiddleware) {
        ...
        if (!_.isString(this._srcProp)){
            throw new EsMiddlewareError(EsGetPublicDataMiddleware.name, 'sourceProp MUST be string');
        }

        if (!_.isUndefined(this._destProp) && !_.isString(this._destProp)){
            throw new EsMiddlewareError(EsGetPublicDataMiddleware.name, 'destProp MUST be string');
        }
    }
    ...
    async runInternal(context: IEsContext) {
        ...
        if (!_.isString(src)) {
            throw new EsMiddlewareError(EsGetPublicDataMiddleware.name, `Destination (prop ${this._srcProp}) MUST be string`);
        }
...
export const MiddlewareCtor: IEsMiddlewareConstructor = EsGetPublicDataMiddleware;
